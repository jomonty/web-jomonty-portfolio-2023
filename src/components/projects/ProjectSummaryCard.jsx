...
const ProjectSummaryCard = ({ project }) => {
  ...
  return (
    <div className="group relative flex flex-col px-5 py-5">
      ...
      <div className="z-10 mt-2 flex flex-col justify-around">
        {project.related_article ? (
          <Link
            to={`/articles/${project.related_article_id}`}
            className="p-3 font-semibold tracking-wider text-logoprimary hover:text-logoprimary sm:text-logosecondary"
          >
            Blog
          </Link>
        ) : null}
